...
z <- (q_data$IR > 0)
y <- ifelse(z == 1, unlist(q_data$IR), NA)
A <- inla.spde.make.A(mesh = mesh_n1, 
                      loc = coords_all, 
                      group = q_data$year_no)
...
res <-  inla(formula, 
                  ...
                  control.predictor = list(A = inla.stack.A(data_stack), link=link), #data_stack$data$data$
                  ...
                  control.fixed = list(expand.factor.strategy = 'inla'), # to deal with factors
                  verbose = F)
